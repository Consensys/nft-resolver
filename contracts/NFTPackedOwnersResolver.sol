...
contract NFTPackedOwnersResolver is
    EVMFetchTarget,
    ITargetResolver,
    IExtendedResolver,
    IAddrSetter,
    ERC165
{
    ...
    function _addr(
        ...
        bytes memory callbackData = abi.encode(tokenId, slot, target);
        EVMFetcher
            .newFetchRequest(verifier, target)
            .getStatic(slot)
            .element(tokenId)
            .fetch(this.addrCallback.selector, callbackData); // recordVersions
    }
    ...
    function addrCallback(
        ...
        uint256 uintValue = abi.decode(values[0], (uint256));

        (uint256 tokenId, uint256 slot, address target) = abi.decode(
            extraData,
            (uint256, uint256, address)
        );

        // For packed owners we need to try all the previous tokenIds to get the owner's address
        if (uintValue == 0 && tokenId > 1 && tokenId <= maxNFTId) {
            uint256 newTokenId = tokenId - 1;
            return _addr(newTokenId, slot, target);
        }

        return abi.encode(address(uint160(uintValue)));
    }
